...
class ModifyAddresseMultiple extends StatefulWidget {
  Map partenaire;
  Map dataAdresse;
  int form_start;
  ...
  @override
  _ModifyAddresseMultipleState createState() => _ModifyAddresseMultipleState();
}

class _ModifyAddresseMultipleState extends State<ModifyAddresseMultiple> {
  int form_number = 1;
  //forPartenaire
  ...
  void initState() {
    setState(() {
      ...
      if (widget.dataAdresse['idPhotoAdresse'] == null) {
        idPhotoAdresse = _photoadresse.doc().id;
        number_of_photo = 0;
        _photoadresse.doc(idPhotoAdresse).set({
          'number_of_photo': '0',
          'idAdresse': widget.dataAdresse['idAdresse'],
        });
        _adresse.doc(widget.dataAdresse['idAdresse']).update({
          'number_of_photo': '0',
          'idPhotoAdresse': idPhotoAdresse,
        });
      } else {
        number_of_photo = int.parse(widget.dataAdresse['number_of_photo']);
        idPhotoAdresse = widget.dataAdresse['idPhotoAdresse'];
      }
    });
    if (number_of_photo != 0) {
      photoUrl = List<String>.generate(number_of_photo, (index) => '');
      for (int i = 0; i < number_of_photo; i++) {
        _photoadresse
            .where('idAdresse', isEqualTo: widget.dataAdresse['idAdresse'])
            .limit(1)
            .get()
      ...
      listFile = List<File>.generate(number_of_photo, (index) => File('zz'));
      listUint8 =
          List<Uint8List>.generate(number_of_photo, (index) => Uint8List(10));
      listImagePicker =
          List<ImagePicker>.generate(number_of_photo, (index) => ImagePicker());
    }
    super.initState();
  }
  ...
  CollectionReference _photoadresse =
      FirebaseFirestore.instance.collection("PhotoAdresse");
  int number_of_photo = 0;
  String idPhotoAdresse = '';
  ...
  Map<String, dynamic> dataphoto = {};

  List<String> photoUrl = [];
  List<File> listFile = [];
  List<Uint8List> listUint8 = [];
  List<ImagePicker> listImagePicker = [];
  List<XFile?> listXFile = [];
  uploadImage({required int index}) async {
  ...
  Widget build(BuildContext context) {
    ...
    double page_photo = MediaQuery.of(context).size.width * 0.95;
    ...
    // For Table Photo
    var list_photo_row = List<Widget>.generate(number_of_photo + 1, (index) {
      return photo_adresse(context: context, index: index);
    });
    return Scaffold(
                                                            ...
                                                            Fluttertoast.showToast(
                                                                ...
                                                                gravity:
                                                                    ToastGravity
                                                                        .TOP);
                                                            Navigator.of(context).pushReplacement(MaterialPageRoute(
                                                                builder: (context) => ModifyAddresseMultiple(
                                                                    partenaire:
                                                                        widget
                                                                            .partenaire,
                                                                    dataAdresse:
      ...
      Visibility(
        visible: form_number == 4,
        child: Align(
          alignment: Alignment(-0.9, 0),
          child: Container(
            height: 500 + 1500 * double.parse(number_of_photo.toString()),
            margin: const EdgeInsets.only(
              left: 20,
              top: 20,
              bottom: 20,
            ...
            child: Column(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.center,
              children: photo_form_title() + list_photo_row,
            ),
          ),
        ),
      ),
  ...
  Widget photo_adresse({
    required BuildContext context,
    required int index,
  }) {
    double row_width = MediaQuery.of(context).size.width * 0.9;
    if (index == number_of_photo) {
      return Container(
        margin: const EdgeInsets.symmetric(vertical: 10),
        width: row_width / 10,
        decoration: BoxDecoration(
          ...
          children: [
            Container(
                ...
                child: IconButton(
                  onPressed: () async {
                    photoUrl.add('');
                    listFile.add(File('zz'));
                    listUint8.add(Uint8List(10));
                    listImagePicker.add(ImagePicker());
                    await uploadImage(index: index);
                    String fileName = widget.dataAdresse['idAdresse'] +
                        '/' +
                        _photoadresse.doc().id +
                        '.png';
                    ...
                    await _photoadresse.doc(idPhotoAdresse).update({
                      'number_of_photo': (number_of_photo + 1).toString(),
                      '$index': signature_url.toString(),
                    });
                    await _adresse.doc(widget.dataAdresse['idAdresse']).update({
                      'number_of_photo': (number_of_photo + 1).toString(),
                    });
                    setState(() {
                      photoUrl[index] = signature_url.toString();
                      number_of_photo++;
                    });
                  },
                  icon: Icon(
      ...
      // print('photoURl $index = ${photoUrl[index]}');
      return Container(
        margin: const EdgeInsets.symmetric(vertical: 10),
        width: row_width,
        decoration: BoxDecoration(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Container(
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          String deleteUrl = photoUrl[index];
                          await FirebaseStorage.instance
                              .refFromURL(deleteUrl)
                              .delete();
                          if (index == number_of_photo - 1) {
                            await _photoadresse.doc(idPhotoAdresse).update({
                              'number_of_photo':
                                  (number_of_photo - 1).toString(),
                              '$index': '',
                            });
                            await _adresse
                                .doc(widget.dataAdresse['idAdresse'])
                                .update({
                              'number_of_photo':
                                  (number_of_photo - 1).toString(),
                            });
                          } else {
                            Map<String, dynamic> update_photo_data = {};
                            for (int i = index; i < number_of_photo - 1; i++) {
                              update_photo_data['$i'] = photoUrl[i + 1];
                              setState(() {
                                photoUrl[i] = photoUrl[i + 1];
                              });
                            }
                            int last_element = number_of_photo - 1;
                            update_photo_data['$last_element'] = '';
                            update_photo_data['number_of_photo'] =
                                (number_of_photo - 1).toString();
                            await _photoadresse
                                .doc(idPhotoAdresse)
                                .update(update_photo_data);
                            await _adresse
                                .doc(widget.dataAdresse['idAdresse'])
                                .update({
                              'number_of_photo':
  ...
  List<Widget> photo_form_title() {
    List<Widget> result = [];
    double row_width = MediaQuery.of(context).size.width * 0.95;
    result.add(Container(
      ...
      child: Text(
        graphique.languagefr['modify_addresse_multiple_page']['subtitle_page'] +
            ': ' +
            widget.dataAdresse['nomPartenaireAdresse'],
        style: TextStyle(
          color: Color(graphique.color['main_color_2']),
